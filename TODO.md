...
- [x] **Precios reales en el token tracker** ✅ (23-09):
  - Orden de resolución: override manual (`system_config.llm.prices`, editable en Consumo → Precios) → catálogo LiteLLM buscando `<preset>/<modelo>` y variantes → fallback por familia → default. Cada registro guarda `price_source`; la GUI marca "≈ aproximado" cuando no vino del catálogo.
  - Catálogo se refresca a diario (antes 7 días y no estaba corriendo: la copia local tenía 2.671 modelos vs 4.215; `gemini-3.8-flash` se tarifaba 0.50/3.00 en vez de 0.75/3.75 y `gemini-3.5-flash-lite` 0.10/0.40 en vez de 0.30/2.50).
  - Tokens en caché (Gemini `cachedContentTokenCount`, OpenAI `prompt_tokens_details.cached_tokens`, DeepSeek `prompt_cache_hit_tokens`, Anthropic `cache_read_input_tokens`) se cobran a su tarifa; tokens de razonamiento se registran (`thoughts_tokens`). Columnas nuevas en `usage_history`.
  - `POST /api/usage/reprice` recalcula el mes con los precios vigentes (el mes en curso pasó de $5,48 a $8,55 al retarifar).
  - Coordinator y public_coordinator reciben la **fecha y hora actual** (America/Santiago) al final de la instrucción: un "¿qué hora es?" costaba 263k tokens / $0,23 (24 pasos por account_agent, commitments y triage); ahora 1 llamada / $0,017.
- [x] **Adjuntos de herramientas (imágenes/archivos) y modelos reales para el builder** ✅ (23-09):
  - Una herramienta devuelve `{ adjuntos: [{ tipo, mime, base64, nombre, caption }] }`; el sandbox lo saca ANTES del tope de tamaño, lo guarda en `data/adjuntos/<id>` (id aleatorio de 128 bits, caduca a 7 días) y al modelo le llega solo `{ id, url, marcador }`. El agente escribe `[[adjunto:ID]]` y cada canal lo convierte: imagen inline en la GUI (`markdown.pipe`), `sendPhoto`/`sendDocument` en Telegram (multipart), enlace público en Google Chat, Buzz y A2A. Ruta pública `GET /api/adjuntos/:id` (URL-capacidad).
  - `list_provider_models` para el agente programador + regla de no adivinar modelos ni endpoints (Banano nació con `imagen-3.0-generate-002:predict`, que ya no existe); receta correcta de Gemini Image (`gemini-*-image:generateContent` + `responseModalities: ["IMAGE"]` + `imageConfig.aspectRatio`). Herramientas con red: 90 s de tope y respuestas de hasta 12 MB.
  - Banano reescrito a `gemini-3.1-flash-image`: imagen 9:16 en el chat en ~10 s por $0,004.
- [x] **Self agents (agentes personalizados creados por IA)** ✅ (23-09):
  ...
  - **Mención directa** (23-09): en GUI, API y Telegram, `@slug …` va directo al agente personalizado sin Coordinator (`customAgentsService.prepararTurno`); misma sesión del canal (el Coordinator ve esos turnos como contexto), el agente solo recibe sus propios turnos `@slug` previos. Buzz y A2A no reconocen la mención: decide el Coordinator. Autocompletado de `@` en el chat y etiqueta "directo" en la respuesta. ~10× más barato que vía Coordinator ($0,0009 vs $0,0095).
